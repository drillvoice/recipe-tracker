...
### Added
- **Calendar Emoji Indicators**: Calendar cells now surface a single leading emoji from that day's meals when available, making it easy to spot themed dishes at a glance.
- **Unit Test Coverage**: Added CalendarView tests ensuring only one emoji renders per day and that emoji extraction falls back gracefully.

### Changed
- **Calendar Layout Tweaks**: Updated calendar day styling to stack the date number with the optional emoji without disrupting existing interactions or accessibility labels.
